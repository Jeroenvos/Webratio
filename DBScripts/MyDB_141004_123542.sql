...
create index `idx_runnerbelongstorunnergroup` on `runnerbelongstorunnergroup`(`runner_oid`);
create index `idx_runnerbelongstorunnergro_2` on `runnerbelongstorunnergroup`(`runnergroup_oid`);
...
create index `idx_runnergroup_runner` on `runnergroup`(`runner_oid`);
...
create index `idx_runnerenrolledintraining_p` on `runnerenrolledintraining_progr`(`runner_oid`);
create index `idx_runnerenrolledintraining_2` on `runnerenrolledintraining_progr`(`training_program_oid`);
...
create index `idx_training_training_program` on `training`(`training_program_oid`);
...
create index `idx_trainingexecution_runner` on `trainingexecution`(`runner_oid`);
...
create index `idx_trainingexecution_training` on `trainingexecution`(`training_oid`);
