...
alter table `training`  add column  `trainingexecution_oid`  integer;
alter table `training`   add index fk_training_trainingexecution (`trainingexecution_oid`), add constraint fk_training_trainingexecution foreign key (`trainingexecution_oid`) references `trainingexecution` (`oid`);
